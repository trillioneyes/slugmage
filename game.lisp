  ...
  (with-accessors ((font home-font) (target target) (grazing grazing)
   ...
   (cond ((not target)
         ...
         ((< (density (pos monster) *world* :dist 2)
             (/ grazing 400))
          (incf food
                (ceiling (/ grazing (num-grazers target))))
...
(defmethod draw ((object world) x0 y0 window)
  (dolist (monster (alexandria:hash-table-values (monsters object)))
    (let ((x (x monster))
          (y (y monster)))
      (if (eq monster (world-at object x y))
...
(defun add-monster (monster world)
  (labels ((find-clear-space (monster world)
             (loop with p = (pos monster) while (world-at world (x p) (y p))
                  do (setf p (move p (random-delta 3) (random-delta 3)))
                  finally (return p))))
